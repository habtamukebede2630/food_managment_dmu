...
$querydailyexp=mysqli_query($conn,"select *from dailyexpenditure");
if($querydailyexp){
    $row =mysqli_fetch_array($querydailyexp);
    $statusexp=$row['status'];
}
?>
<html>
<head>
...
$currentstudent=$totalstud-($noncafe+$internship+$punish);
	...
	$sql=mysqli_query($conn,"select * FROM dailyexpenditure WHERE day='$dattee' and status='A'") or die(mysqli_error());
...
$currentstudent=$totalstud-($noncafe+$internship+$punish);
	...
	while($row5=mysqli_fetch_array($result6))
	{
	 ...
	 $dat=$date;
		...
		$sqld=mysqli_query($conn,"SELECT * FROM dailyexpenditure WHERE date='$date' and itemname='$item' and status='A'") or die(mysql_error());
	...
	//$sql3="INSERT INTO dailyexpenditure(date,itemname,dailyexpenditure,day,senderid,fname,lname,photo,status) VALUES('$dat','$item','$daily','$day','$id','$name','$lname','$img','A')";
	   $updatedaiy=mysqli_query($conn,"update dailyexpenditure set status ='approve' where itemname='$item'");
	//$=mysqli_query($conn,$sql3) or die(mysql_error()); 
   ...
   else if($statusexp=='approve'){
        echo '<script type="text/javascript">
	alert(" approved previously")</script>';
