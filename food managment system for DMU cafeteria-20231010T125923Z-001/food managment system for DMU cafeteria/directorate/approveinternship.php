...
$img=$query2['photo'];
	...
	if(isset($_POST['update']))
	{
		...
		$sqlo=mysqli_query($conn,"update internship set status='approve' where department='$dept' and year='$year'");
		if($dif>30)
		{
	...
	alert("birr added to kazina!")</script>'.mysqli_error();	
		}
		...
		echo '<script type="text/javascript">
	alert(" sorry you were add to the kazina before")</script>';
		}	
	...
	$result=mysqli_query($conn,"select * from internship where no='$userid' ")or die(mysql_error()); 
...
<form action="" method="post">
 <div class="inner-wrap">
	<label>Request ID<input type="text" name="userid" value="<?php echo $row['no'];?>" readonly></label>
